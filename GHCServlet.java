...
public class GHCServlet extends HttpServlet {
	...
	private String alert(String lat, String lng) {
		...
		try {
			...
			if (jdbcDriver.contains("postgres")) {
				sql = "SELECT commuteid, latcurr, lngcurr, round(cast( 6371 * acos( cos( radians(" + lat + ") ) * cos( radians( latcurr ) ) "
						+ "* cos( radians( lngcurr ) - radians(" + lng +") ) + sin( radians("+ lat + ") ) * "
						+ "sin( radians( latcurr ) ) ) as numeric),2) AS distance FROM commute ORDER BY distance OFFSET 0 LIMIT 25";
			}
	...
	private String getHospitals(String lat, String lng) {
		...
		try {
			...
			if (jdbcDriver.contains("postgres")) {
				sql = "SELECT hospid, hospname, lat, lng, phone, round(cast( 6371 * acos( cos( radians(" + lat + ") ) * cos( radians( lat ) ) "
						+ "* cos( radians( lng ) - radians(" + lng +") ) + sin( radians("+ lat + ") ) * "
						+ "sin( radians( lat ) ) ) as numeric),2) AS distance FROM hospitals ORDER BY distance OFFSET 0 LIMIT 10";
			}
			System.out.println("sql: " + sql);
			ResultSet rs = stmt.executeQuery(sql);
			details = "{\"hospitals\":[";
